...
## Bonus: The Corepack Nightmare

And just when you think it can't get worse, you hit the Corepack wall.

Your project uses pnpm. You have Corepack installed on your Node version. You run `pnpm install` on the server and... it refuses to load pnpm. Corepack says pnpm is enabled, but it just... doesn't work. No error message. No helpful hint. Just vibes.

So what do you do? You resort to this cursed command:

```bash
sudo npx pnpm install
```

You would think this shouldn't work. `npx` running `pnpm` which then installs dependencies? With `sudo` because why not add more chaos? But somehow, against all logic and reason, it does work.

This is what manual deployment does to you. It makes you write commands that look like they were generated by a Markov chain trained on Stack Overflow answers from 2019.
