  ...
  return (
    <nav className="container sticky top-0 flex flex-col md:flex-row p-4 text-xl">
      <div className="flex grow justify-between items-center text-black dark:text-white">
        <Link
          href="/"
          className="inline transition-all hover:text-blue-400 hover:scale-110"
        >
        ...
        <div className="inline-flex gap-4 md:mr-4">
          ...
          <Dialog.Root open={isOpen} onOpenChange={setOpen}>
            ...
            <Dialog.Portal>
              <Dialog.Backdrop
                className={'fixed backdrop-blur-xs inset-0 z-50'}
              />
              <Dialog.Popup
                className={
                  'fixed top-3 left-0 right-0 p-2 max-w-full mx-4 rounded-2xl bg-blue-300 dark:bg-gray-700 z-50'
                }
              >
                <div
                  className={
                    'flex flex-row justify-between items-center text-black dark:text-white'
                  }
                >
                  <h6 className={'text-lg font-bold'}>Djoni&apos;s Den</h6>
                  <Dialog.Close
      ...
      <div
        className={'hidden md:flex md:flex-row gap-4 transition duration-500'}
      >
        <Navlinks />
      </div>
    </nav>
